...
"""def update_U(data, vs_from_u, x, k, lam):
    ...
    for a in range(len(vs_from_u)):
        ...
        I_a = [vs_from_u[a][l_a][0] for l_a in range(len(vs_from_u[a]))]

        if len(I_a) == 0:
            continue

        B_a = np.hstack([v[i] for i in I_a])
        Z_a = np.array([[vs_from_u[a][l_a][1] for l_a in range(len(vs_from_u[a]))]]).T
        ba_v = np.array([np.take(b_v, I_a)]).T
        T_a = Z_a - ba_v
        
        u[a], b_u[a] = ridge_analytic(B_a.T, T_a, lam)
...
def update_V(data, us_from_v, x, k, lam):
    ...
    for a in range(len(us_from_v)):
        
        I_a = [us_from_v[a][l_a][0] for l_a in range(len(us_from_v[a]))]

        if len(I_a) == 0:
            continue

        B_a = np.hstack([u[i] for i in I_a])
        Z_a = np.array([[us_from_v[a][l_a][1] for l_a in range(len(us_from_v[a]))]]).T
        ba_u = np.array([np.take(b_u, I_a)]).T
        T_a = Z_a - ba_u
        
        v[a], b_v[a] = ridge_analytic(B_a.T, T_a, lam)
...
# Simple validate case
# mf_sgd(ratings_small, ratings_small, step_size_fn=lambda i: 0.1, lam=0.01, max_iter=1000, k=2)
